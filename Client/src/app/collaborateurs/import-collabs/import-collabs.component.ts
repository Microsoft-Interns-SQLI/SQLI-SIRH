...
export class ImportCollabsComponent implements OnInit {
  ...
  uploadFile() {
    this.isLoading = true;
    this.collabService.importCollaborateurs(this.formData).subscribe({
      next: (event) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          if (event.ok) {
            this.messages.push(
              `${event.body.ExistsCollab} Collaborateurs existaient déjà dans la BD !`
            );
            this.messages.push(
              `${event.body.AddingCollab} Collaborateurs ajoutés à la bd!`
            );
            this.error = '';
          }
        }
